...
def _make_causal_mask(input_ids_shape: torch.Size, dtype: torch.dtype, past_key_values_length: int = 0):
    ...
    # xjw: keypoint producing real mask
...
class BartEncoder(BartPretrainedModel):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            # print(attention_mask) 
            # print(attention_mask.shape) 1 1 seq seq 
...
class BartDecoder(BartPretrainedModel):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        ...
        if input_shape[-1] > 1:
            # xjw: producing ddecoder mask fpr training
        ...
        if self.training:
            ...
            # threshold = torch.linspace(0, 0.6, seq_len, device=self.device).reshape(1, 1, -1, 1).expand(*mask_shape[: 2], -1, -1)
            specific_mask = torch.rand(size=(*mask_shape[:2], 1, seq_len), device=self.device)
            mask_rate = torch.linspace(0, 0.8, seq_len, device=self.device)
            specific_mask = torch.where(specific_mask < mask_rate, -1e4, 0.0)
            specific_mask = specific_mask.expand(-1, -1, seq_len, -1)
            # specific_mask[:, :, :, 0] = 0
            # specific_mask[1:, :, :, :] = 0
            # specific_mask[:, :, 0: 4, :] = 0
            combined_attention_mask = specific_mask + combined_attention_mask
    ...
    def forward(
        ...
        # print(attention_mask)
        # print(attention_mask) xjw: produce inf or 0
...
class BartSimMCS(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head\.weight"]
