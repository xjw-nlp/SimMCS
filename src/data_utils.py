...
class CustomDataset(Dataset):
    def __init__(self, fdir, model_type, max_len=-1, is_test=False, total_len=512, is_sorted=True, max_num=-1, is_untok=True, is_pegasus=False, num=-1):
        ...
        self.num_files = len(os.listdir(self.fdir))
    ...
    def __getitem__(self, idx):
        ...
        cand_txt = [" ".join(abstract)] + [" ".join(x[0]) for i, x in enumerate(candidates)]
        pos_weights = [x[1] for x in candidates]

        if not self.is_test:
            neg_cand_txt = []
            sampled_ids = random.sample(range(self.num_files), k=2)
            if idx in sampled_ids:
                sampled_ids.remove(idx)
            else:
                sampled_ids = sampled_ids[: -1]
            for neg_idx in sampled_ids:
                with open(os.path.join(self.fdir, '%d.json' % neg_idx), 'r') as f:
                    neg_data = json.load(f)
                if self.is_untok:
                    neg_abstract = neg_data['abstract_untok']
                else:
                    neg_abstract = neg_data['abstract']
                # rouge_scores = self.rouge.compute(predictions=['\n'.join(neg_abstract)], references=['\n'.join(abstract)])
                # neg_cand_txt.append([' '.join(neg_abstract), rouge_scores['rouge1']])
                neg_cand_txt.append(' '.join(neg_abstract))
            cand_txt += neg_cand_txt
...
def collate_mp(batch, pad_token_id, is_test=False):
    ...
    pos_weights = torch.Tensor([x['pos_weights'] for x in batch])
    if is_test:
