...
from data_utils import to_cuda, collate_mp, CustomDataset
from torch.utils.data import DataLoader
import torch.distributed as dist
import torch.multiprocessing as mp
from functools import partial
from model import SimMCS
import logging
from loss_utils import label_smoothing_loss, RankingLoss
from nltk import sent_tokenize, word_tokenize
from config import cnndm_setting, xsum_setting, cnndm_cased_setting
...
def evaluation(args):
    ...
    if args.do_generation:
        ...
        total_num = len(os.listdir(os.path.join(root_path, f"{args.dataset}/{args.datatype}/test")))
        with open(os.path.join(root_path, f'{args.dataset}/{args.datatype}/test.source')) as source, open(os.path.join(root_dir, "test.out"), 'w') as fout:
        ...
        with open(f"{root_dir}/test.out") as fout, open(os.path.join(root_path, f'{args.dataset}/{args.datatype}/test.target')) as target:
...
def run(rank, args):
    ...
    if args.config == "cnndm":
        cnndm_setting(args)
    elif args.config == "cnndm_cased":
        cnndm_cased_setting(args)
    elif args.config == "xsum":
        xsum_setting(args)
    else:
        base_setting(args)
    ...
    train_set = CustomDataset(os.path.join(root_path, args.dataset, args.datatype, "train"), args.model_type, max_len=args.max_len, max_num=args.max_num, total_len=args.total_len, is_pegasus=args.is_pegasus)
    val_set = CustomDataset(os.path.join(root_path, args.dataset, args.datatype, "val"), args.model_type, is_test=True, max_len=512, is_sorted=False, max_num=args.max_num, total_len=args.total_len, is_pegasus=args.is_pegasus)
    if is_mp:
    ...
    # build models
    model_path = args.pretrained if args.pretrained is not None else args.model_type
    model = SimMCS(model_path, tok.pad_token_id, is_pegasus=args.is_pegasus)
    if len(args.model_pt) > 0:
    ...
    for epoch in range(args.epoch):
        ...
        for (i, batch) in enumerate(tqdm(dataloader)):
            ...
            pos_constraint = torch.mean(pos_weights) * 100
            output = model(batch["src_input_ids"], batch["candidate_ids"], args.normalize, args.score_mode, args.length_penalty, adding=args.adding)
         
            similarity, gold_similarity = output['score'], output['summary_score']

            mix_similarity = output['all_score']
            mix_similarity = mix_similarity * args.scale
            pos_cand_mix_similarity = mix_similarity[:, 1: -1]
            neg_cand_mix_similarity = mix_similarity[:, -1:]
            
            similarity = similarity * args.scale
            gold_similarity = gold_similarity * args.scale
            ranking_loss = RankingLoss(pos_cand_mix_similarity, gold_similarity, args.margin, args.gold_margin, args.gold_weight)

            probs = output["probs"]  # [bz, seq_len, word_num]
            probs = output["probs"][:, :-1]  # truncate last token
            gold = batch["candidate_ids"][:, 0, 1:]  # shift right

            pos_avg = torch.mean(torch.sum(pos_cand_mix_similarity * pos_weights, dim=-1) / torch.sum(pos_weights + 1e-9, -1))
            neg_avg = torch.mean(neg_cand_mix_similarity)
            pos_neg_loss = -torch.log(torch.sigmoid(pos_constraint * pos_avg - neg_avg))
            mle_loss = mle_fn(probs.transpose(1, 2), gold)
            
            loss = args.rank_weight * ranking_loss + args.mle_weight * mle_loss + args.pos_neg_weight * pos_neg_loss
            avg_pos_neg_loss += pos_neg_loss.item() / args.accumulate_step
            avg_mle_loss += mle_loss.item() / args.accumulate_step
            avg_ranking_loss += ranking_loss.item() / args.accumulate_step
            avg_pos_avg += pos_avg.item() / args.accumulate_step
            avg_neg_avg += neg_avg.item() / args.accumulate_step
            ...
            if epoch_step % args.report_freq == 0 and step_cnt == 0 and is_master:
                ...
                recorder.print()
                if args.enable_wandb:
                    wandb.log({'loss': avg_loss / args.report_freq, 'mle_loss': avg_mle_loss / args.report_freq, 
                                "ranking_loss": avg_ranking_loss / args.report_freq, 'gold_pos_loss': avg_gold_pos_loss / args.report_freq,
                                'pos_neg_loss': avg_pos_neg_loss / args.report_freq, 'learning_rate': lr,
                                'pos_avg': avg_pos_avg / args.report_freq, 'neg_avg': avg_neg_avg / args.report_freq})
                    
                avg_gold_pos_loss, avg_pos_neg_loss, avg_mle_loss, avg_ranking_loss, avg_loss = 0, 0, 0, 0, 0
                avg_pos_avg, avg_neg_avg = 0, 0
            
            del similarity, gold_similarity, loss, mle_loss, ranking_loss, output, probs, mix_similarity, pos_cand_mix_similarity, pos_neg_loss, neg_cand_mix_similarity
            ...
            if all_step_cnt % args.eval_interval == 0 and all_step_cnt != 0 and step_cnt == 0:
                ...
                if is_master:
                    ...
                    if args.enable_wandb:
                        wandb.log({'rouge1': result["rouge1"], 'rouge2': result["rouge2"], "rougeL": result["rougeLsum"]})
                ...
                if is_master:
                    ...
                    if args.do_sample:
                        ...
                        if args.enable_wandb:
                            wandb.log({'gen_rouge1': result["sample_rouge1"], 'gen_rouge2': result["sample_rouge2"], "gen_rougeL": result["sample_rougeLsum"]})
...
if __name__ ==  "__main__":
    ...
    parser.add_argument("-p", "--port", type=int, default=12357, help="port")
